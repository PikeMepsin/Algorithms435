...
class Location implements Comparable<Location>{
  ...
  void start() {  
    // Calling start initiates the solving
  ...
  Location nextNeighbor() {  
    ...
    if (iterationMode == CHANGE_LETTER) {
      if (indexToChange < temp.length()) {
        if (nextLetter == temp.charAt(indexToChange)) {
          nextLetter++;
        }
        temp.setCharAt(indexToChange, nextLetter);
        Scanner nextWord = new Scanner(temp.toString());
        next.streamIn(nextWord);
        nextLetter++;
    ...
    if (iterationMode == DELETE_LETTER) {
      if (indexToChange < temp.length() && temp.length() > 1){
        temp.deleteCharAt(indexToChange);
        Scanner nextWord = new Scanner(temp.toString());
        next.streamIn(nextWord);
        indexToChange++;
      }
